...
async function createCell(block_id, capacity, actual_capacity, category, cell_number) {
  await poolConnect;
  const result = await pool
    .request()
    ...
    .query(`
      INSERT INTO cells (block_id, capacity, actual_capacity, category, cell_number) 
      VALUES (@block_id, @capacity, @actual_capacity, @category, @cell_number)
    `);
  return result;
}

async function updateCell(id, block_id, capacity, actual_capacity, category, cell_number) {
  await poolConnect;
  const result = await pool
    .request()
    .input("id", sql.Int, id)
    ...
    .query(`
      UPDATE cells 
      SET block_id = @block_id, capacity = @capacity, actual_capacity = @actual_capacity, category = @category, cell_number = @cell_number 
