...
const CellsList = ({ cells, blocks, onEdit, onDelete, goToCreate }) => {
  ...
  const currentCells = filteredCells.slice(indexOfFirst, indexOfLast);
  const totalPages = Math.ceil(filteredCells.length / itemsPerPage);
        ...
        <div className="card-body">
          ...
          <div className="table-responsive">
            <table className="table align-middle">
              <thead className="table-light">
                <tr>
                  {[
                    { field: "cell_block_ID", label: "ID" },
                    { field: "block_id", label: "Block Name" },
                    { field: "cell_number", label: "Cell Number" },
                    { field: "capacity", label: "Capacity" },
                    { field: "actual_capacity", label: "Actual Capacity" },
                ...
                {currentCells.length > 0 ? (
                  currentCells.map((cell) => (
                    <tr key={cell.cell_block_ID}>
                      ...
                      <td>{getBlockName(cell.block_id)}</td>
          ...
          <div className="d-flex justify-content-between align-items-center mt-3 flex-wrap">
            <div className="small text-muted mb-2 mb-md-0">
              Showing {indexOfFirst + 1} to {Math.min(indexOfLast, filteredCells.length)} of{" "}
              {filteredCells.length} entries
            </div>
            ...
            <div className="modal-dialog">
              <div className="modal-content">
                ...
                <div className="modal-body">
                  <p>Are you sure you want to delete this cell?</p>
                </div>
